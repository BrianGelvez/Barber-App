...
export const CalendarJulianUsers = () => {
  ...
  const scheduleAppointment = () => {
    const newAppointment = {
      ...
      date: date.toLocaleString(), // Formatear la fecha según tus necesidades
    };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.text}>Seleccionado: {date.toLocaleString()}</Text>
      {show && (
        <DateTimePicker
      ...
      <View style={styles.appointmentsContainer}>
        <Text style={styles.heading}>Citas Agendadas:</Text>
        {appointments.map((appointment) => (
          <Text key={appointment.id}>{appointment.date}</Text>
        ))}
      </View>
    </SafeAreaView>
  );
...
//   const [mode, setMode] = useState<'date' | 'time' | 'datetime'>('date');
//   const [show, setShow] = useState(false);
//   const { selectedBarber } = useUserContext();
//   console.log( 'peluquero:', selectedBarber);
